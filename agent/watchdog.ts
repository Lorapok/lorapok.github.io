...
interface WatchdogState {
  ...
  serverStatus: 'running' | 'down' | 'restarting' | 'unknown';
  lastHealthcheckAt: string | null;
  lastDispatchAt: string | null;
  postsInCatalog: number;
...
function probeHealth(): Promise<{ ok: boolean; data?: any }> {
...
function triggerDispatch(): Promise<boolean> {
  return new Promise((resolve) => {
...
function startBackendServer() {
  ...
  serverProcess = spawn(process.execPath, [serverPath, '--daemon'], {
...
async function auditCadenceAndCatchUp() {
  ...
  try {
    ...
    const posts: any[] = JSON.parse(fs.readFileSync(POSTS_JSON_PATH, 'utf8'));
...
export async function runWatchdog(loop = true) {
  console.log("=================================================================");
