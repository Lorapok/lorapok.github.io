...
export interface ReviewVerdict {
  ...
  score: number; // 0 - 100 (Threshold: 85)
  category: string;
  editorialType: string;
  rubricBreakdown: {
    architecturalDepth: number;     // max 30
    structuralCompleteness: number; // max 25
    codeAndMathSoundness: number;   // max 20
    citationsIntegrity: number;     // max 15
    antiHallucinationTone: number;  // max 10
  };
  strengths: string[];
 ...
 */
export function heuristicAudit(title: string, content: string, citations: any[] = []): ReviewVerdict {
...
export async function auditArticle(
  article: { title: string; content: string; citations?: any[]; category?: string; type?: string }
