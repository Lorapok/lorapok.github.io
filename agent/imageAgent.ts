...
import { resolveTechnicalTheme } from './imageGen';

export interface PostVisualSuite {
  coverImage: string;
  ...
  figures: Array<{
    ...
    url: string;
    mermaid?: string;
    type: 'architecture' | 'benchmark' | 'concept';
  }>;
}
 ...
 */
export function synthesizeMermaidArchitecture(
  title: string,
  category: string = 'Backend & Infrastructure',
  tags: string[] = []
): string {
 ...
 */
export function generateBenchmarkSvg(title: string, category: string = 'Systems Engineering'): string {
 ...
 */
export function generateBenchmarkSvgDataUri(title: string, category: string = 'Systems Engineering'): string {
 ...
 */
export async function synthesizePostVisualSuite(
  title: string,
  category: string,
  tags: string[] = [],
  ...
  for (const p of existingPosts) {
    if (p.coverImage) usedUrls.add(p.coverImage.trim());
    if (p.architectureImage) usedUrls.add(p.architectureImage.trim());
    if (p.benchmarkImage) usedUrls.add(p.benchmarkImage.trim());
    if (Array.isArray(p.figures)) {
      p.figures.forEach((f: any) => f.url && usedUrls.add(f.url.trim()));
    }
  }
 ...
 */
export function injectVisualsIntoMarkdown(content: string, visuals: PostVisualSuite): string {
