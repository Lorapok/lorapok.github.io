 ...
 */
export function calculateOverlapCoefficient(str1: string, str2: string): number {
  const words1 = extractKeywords(str1);
  const words2 = extractKeywords(str2);
  if (words1.length === 0 || words2.length === 0) return 0;
 ...
 */
export function extractTopicPrefix(title: string): string {
 ...
 */
export function validateDuplicatePost(
  candidate: PostIdentifier,
  existingPosts: any[],
