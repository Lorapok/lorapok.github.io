...
export interface KeyProfile {
  id: string;
  provider: 'gemini' | 'groq' | 'cerebras' | 'sambanova' | 'mistral' | 'github' | 'openrouter' | 'generic';
  key: string;
  accountLabel: string;
  isExhausted: boolean;
...
class KeyManager {
  ...
  public initializeKeys() {
    ...
    // Primary fallbacks
    if (process.env.GEMINI_API_KEY) geminiKeys.push(process.env.GEMINI_API_KEY.trim());
    if (process.env.GOOGLE_API_KEY) geminiKeys.push(process.env.GOOGLE_API_KEY.trim());
  ...
  private registerKey(provider: KeyProfile['provider'], key: string, label: string) {
   ...
   */
  public getKey(provider: string = 'gemini'): KeyProfile | null {
    const prov = (provider === 'google' || provider.startsWith('gemini')) ? 'gemini' : provider as KeyProfile['provider'];
   ...
   */
  public reportRateLimit(keyId: string, isDailyQuota: boolean = false) {
    for (const pool of this.keys.values()) {
      const found = pool.find(p => p.id === keyId);
      if (found) {
  ...
  public getAccountCount(provider: string = 'gemini'): number {
    const pool = this.keys.get(provider as any);
